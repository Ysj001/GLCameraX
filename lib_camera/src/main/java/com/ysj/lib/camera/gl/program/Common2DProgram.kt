 ...
 */
open class Common2DProgram(asset: AssetManager) : AbsGLProgram() {
    ...
    override val program: Int by lazy(LazyThreadSafetyMode.NONE) {
        ...
        val fragSource = asset.open("glsl/common_2d.frag").use {
            it.reader().readText()
        }
        createProgram(
    ...
    protected val u2DTexture by lazy(LazyThreadSafetyMode.NONE) {
        GLES20.glGetUniformLocation(program, "uOESTexture").also {
            checkEGLError("glGetAttribLocation -- u2DTexture")
        }
    }

    override fun run() {
        val texture = this.texture ?: return
        require(texture.target == GLES20.GL_TEXTURE_2D)
        // 使用该程序
    ...
    open fun draw(texture: GLTexture) {
        ...
        checkEGLError("glUniform1i -- u2DTexture")
        // 绘制
